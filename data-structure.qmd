...
Data frames are two-dimensional like matrix and are the most common way to store data. They are similar to Excel spreadsheets. What makes data frames different from matrix is their ability to store different data types. To create a data frame, we use the `data.frame()` function.
...
Below is a simple data frame that stores the inventory of a fruit in a store.
