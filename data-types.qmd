...
1. Create a character variable to store the name of a tree species, `Pinus sylvestris`.

  * Verify that this is a character variable using the class() function. What data type does R return?
...
4. Create a logical variable `protected_area `that stores whether a certain plot of land is a protected area (TRUE).

  *  Use the class() function to verify that protected_area is logical.
  
5. Suppose the types of vegetation in a nature reserve can be categorized into "forest", "grassland", and "wetland". Create a factor variable `vegetation_type` using these categories.

  * Check the structure of the `vegetation_type` variable using str(). What data type is returned?
