...
:::
Before performing any analysis, it's crucial to understand the types of data you are working with. Like data scientist or analysts, researchers must work with various data forms--whether they are survey response, secondary data, field data, or lab data. In the research process, as seen in @fig-str, data collection and analysis are key stages. Each data has its own unique structure and requirements influencing the method you'll use for their analysis. Understanding the distinctions between different data types ensures you can clean, manipulate, and interpret your dataset effectively. In this section, we’ll explore the foundational data types in R programming.

```{r}
#| echo: false
#| label: fig-str
#| fig-cap: "Stages of Research"

library(DiagrammeR)
grViz(
  "digraph {
    graph [layout = dot, rankdir = TB, splines = TRUE]
    node [shape = box, style = filled, fillcolor = white, color = dodgerblue, fontcolor = gray4]
    a [label = 'Identifying the Research Problem']
    b [label = 'Literature Review']
    c [label = 'Formulating Hypothesis or Research Questions']
    d [label = 'Design Research Methodology']
    e [label = 'Collecting Data', color = red, fillcolor = indianred3, fontcolor = white]
    f [label = 'Data Analysis', color = red, fillcolor = indianred3, fontcolor = white]
    g [label = 'Interpretation of Result']
    h [label = 'Drawing Conclusion and Recommendation']
    i [label = 'Report / Presentation to Academic Community and/or Stakeholders']
    a -> b -> c -> d -> e -> f -> g -> h -> i
  }"
)
```
...
There are five different data types in R programming, four which we will be using in this book. In R programming we use the function `typeof` and `class()` to check the type of data a variable is having. The data types are in R programming are:
...
## Character
In R programming, characters are things enclosed in quotation mark. This quotation mark can either be the single  `'` or double `"`, they are treated the same way in R programming. It is more common to use the `"` than the single. Characters while often thought of as alphabets can also be numbers as long as they are enclosed in quotation marks. For example, *"man"* is a character:
...
This is also true if values are assigned to a variable. The content of the variable will be evaluated and not the variable in its self. To put in simple terms, a bottle in itself is not evaluated but the content of the bottle, be it water, wine, or soft drink.
```{r}
man <- "olamide"
class(man)
```

Let's look at the example below, we have error because `olamide` is an object with no values assigned to it. So `olamide` and "olamide" are different. The latter is a character while the formal is an object.
```{r}
#| eval: false

class(olamide)
```
`Error: object 'olamide' not found`.
