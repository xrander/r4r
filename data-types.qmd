...
Researchers regardless of the field are scientist working with various data forms, from survey responses, secondary data, Lab or field data. These data are usually a combination of numbers in the form of a measurements, words or text, such as survey participants responses, or a category or ranking such as yes/no response or response such as marital status. Before performing any analysis, it's crucial to understand the types of data you are working with. In the research process, as seen in @fig-str, data collection and analysis are key stages.
...
There are about seven different data types in R, five which we will be using more in this book. In R programming, we use the function `typeof` and `class()` to check the type of data a variable is having. The data types are in R are:
...
Characters are text and in R they are anything enclosed in quotation marks. This quotation mark can either be the single `'` or double `"`. It is more common to use the double, `"`, than the single, `'`. For example
...
Characters while often thought of as alphabets can also be numbers as long as they are enclosed in quotation marks. For example:

```{r}
class("2")
```

Even when symbols, when enclosed in quotation marks are strings

```{r}
class("+")
```

This is also true if values are assigned to a variable. The content of the variable will be evaluated and not the variable in its self. To put in simple terms, the variables are references to the values.
...
The levels remains the same even on increasing the length of the data here. Other examples of nominal data are:
