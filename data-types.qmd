...
When we assign characters to a variable, the content of the variable will be evaluated and not the variable in its self. As explained in @sec-operators variables are references and not a value in themselves.
...
1:15
```
As `is.integer()` returns true for all integers and `is.double()` returns true for doubles, `is.numeric()` returns true for all both types of data.
```{r}
is.numeric(5L)
is.numeric(3.5)
```
...
Under comparison operators in @sec-operators, I said that comparison operators always returns a logical operator as their result. We can test that here.
...
## Complex
We are not using the complex data type and you are unlikely to analyze these data types. Complex data types are numbers with an imaginary term `i` added to them
...
Characters can be changed back to numeric or double data type using the `as.double()`, `as.integer()` or `as.numeric()` function. The functions will return numbers only if what is been converted contains only digits, else NA is returned as the result. You get a warning when there's not a digit in what is being converted.
...
You can also convert factors to integers as said in @sec-fcts, factors are built on integers.
...
::: callout-note
Data coercion/conversion can either be implicit or explicit. Coercion is explicit when you intentionally change the variable type. Implicit coercion is when R changes the variable type for you automatically depending on when an operation needs it. 
:::
